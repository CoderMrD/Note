...
python多线程支持两种方式来创建线程：一种是继承Thread类，重写他的run()方法，另一种是创建一个threading.Thread对象，在它的初始化函数中(__init__())中可将调用对象作为参数传输。

### 建议49：使用Queue使多线程编程更安全
多线程使用过程中很容易出现错误，线程中的同步个互斥，线程间数据的共享等都是设计线程安全要考虑的问题。
...
### 建议50： 利用模块实现单例模式
GOF的23种设计模式中，单例是最常用的模式，通过单例模式可以保证系统中一个类只有一个实例而且该实例易于被外界访问，从而方便对实例个数的控制并节约系统资源。
模块才用的就是天然的单例的实现。
- 所有的变量都会绑定到模块
- 模块只初始化一次
- import机制是线程安全的（保证了并发状态下模块也只有一个实例）
...
- 修正代码缺陷，重新测试知道代码可接受为止
2) 遵循单元测试基本原则
- 一致性。执行一千次和一次结果应该是一样的
- 原子性。 单元测试的执行结果返回只有两种，True或者False，不存在部分成功或者失败
- 单一职责。测试应基于情景和行为，而不是方法。如果一个方法对应着多种行为，应该有多个测试用例；而一个行为即使对应多个方法也只能有一个测试用例。
- 隔离性。不依赖于具体的环境设置，也不依赖于其他的测试用例以及测试执行的顺序，并且无条件逻辑依赖。所有的输入应该是确定的，方法的行为和结果应是可以预测的。
3) 使用单元测试框架。单元测试常见框架有PyUnit。在python2.1之前需要单独安装，之后成为了一个标准库，名为unittest。支持单元测试自动化。
相关概念：
- 测试固件
- 测试用例
- 测试用例集
